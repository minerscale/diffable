...
//!
//! A [`Bounded`] chart domain and [`BuildNodes`] implementation produce the
//! nodes consumed by [`NerveComplex`]. Their overlaps determine a
//! [`NerveTopology`], whose fundamental group is exposed as a
...
/// which pairs are worth the geometric check. It never prunes alone. The
/// resulting value is cached as part of [`NerveTopology`].
#[derive(Debug, Clone)]
...
/// A stack-backed [`GroupPresentation`] generated by [`group_presentation!`](crate::group_presentation).
///
/// Relation letters occupy one contiguous static array, while [`StaticWord`]
/// values retain the addressable relation boundaries required by
...
/// Declares a static group presentation from indexed generators and relation
/// words.
///
/// A presentation
///
...
/// ```
///
/// is written as a generator count followed by its relations. Each letter in a
/// relation is represented by `(generator, inverted)`:
///
/// - `(i, false)` denotes `gᵢ`;
/// - `(i, true)` denotes `gᵢ⁻¹`.
///
/// For example, the fundamental group of the torus has two generators with the
/// commutator relation `aba⁻¹b⁻¹ = 1`:
///
/// ```
/// use diffable::{
///     group_presentation,
///     traits::simplicial::GroupPresentation,
/// };
///
/// group_presentation!(
///     TORUS,
...
/// );
///
/// assert_eq!(TORUS.n_generators(), 2);
/// assert_eq!(TORUS.relations().len(), 1);
/// ```
///
/// The declaration may include a visibility modifier:
///
/// ```text
/// group_presentation!(
///     pub TORUS,
...
/// # Representation
///
/// The macro expands to a [`StaticGroupPresentation`]. Every letter of every
/// relation is stored in one contiguous static array, while each relation is
/// exposed as a [`StaticWord`] borrowing its corresponding sub-slice. Relation
/// boundaries and offsets are computed at compile time; constructing or
/// traversing the presentation requires no allocation.
///
/// Empty relation words and an empty relation list are representable. The macro
/// preserves relation and letter order exactly as written; it does not freely
/// reduce words, canonicalise the presentation, or remove redundant relations.
///
/// # Validity
///
/// Every generator index appearing in a relation must be less than
/// `n_generators`. This condition is the caller's responsibility; the macro
/// records the presentation but does not perform bounds validation.
///
/// Declaring a presentation does not implement [`Group`] or solve its word
/// problem. It produces finite generator-and-relation data implementing
/// [`GroupPresentation`]. In general, deciding whether two words represent the
/// same element of a finitely presented group is undecidable.
///
...
macro_rules! group_presentation {
    (
        ...
        $vis static $name: $crate::traits::simplicial::StaticGroupPresentation = {
            // one flat array: every relation's pairs, concatenated
            static FLAT: &[(usize, bool)] = &[
                $( $( ($g, $inv) ),* ),*
            ];
            ...
            const fn build_words() -> [$crate::traits::simplicial::StaticWord; LENS.len()] {
                let mut out = [$crate::traits::simplicial::StaticWord(&[]); LENS.len()];
                let mut i = 0;
                while i < LENS.len() {
                    let start = OFFSETS[i];
                    let end = OFFSETS[i + 1];
                    ...
                    out[i] = $crate::traits::simplicial::StaticWord(word);
                    i += 1;
                }
                out
            }
            static WORDS: [$crate::traits::simplicial::StaticWord; LENS.len()] = build_words();

            $crate::traits::simplicial::StaticGroupPresentation {
                n_generators: $n,
                flat_words: FLAT,
                words: &WORDS,
            }
...
impl<F> GeodesicCertificate<F> {
    /// Returns whether this certificate proves global minimality.
...
pub struct Geodesic<P, F> {
    /// The minimizing polyline, or `None` when no candidate was produced.
    pub path: Option<Vec<P>>,
    /// The length of `path`, or the search's best available bound on failure.
    pub length: F,
    /// Evidence describing whether `path` is certified globally minimizing.
    pub certificate: GeodesicCertificate<F>,
}

/// Outcome of the local straightening phase used by
/// [`NerveComplex::geodesic_path`].
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub enum StraighteningResult<F> {
    /// Every candidate requiring inspection was successfully straightened.
    Success,
    /// Straightening ceased to improve after the recorded iteration.
    Stalled(usize),
    /// Numerical resolution reached the recorded nonzero arithmetic floor.
    ArithmeticFloor(F),
    /// Straightening exhausted its iteration budget without convergence.
    NotConverged,
    /// The nerve contains no path connecting the requested endpoints.
    NotConnected,
    /// Adaptive subdivision exhausted its rescue budget.
    MaxRescues,
}

/// A polyline with its cumulative arc length, so sampling is `O(log n)`.
///
/// The global search uses this representation while straightening candidates
/// into the paths returned by [`Geodesic`].
#[derive(Debug, Clone)]
...
///
/// Implementors combine a [`Nodes`] collection with the bounds required by
/// overlap detection, local straightening, and branch-and-bound geodesic
/// search. The defaults are suitable for the canonical covers; individual
/// manifolds may override them when their geometry provides sharper limits.
...
>: NerveComplexParameters<P, V, T, B>
{
    ...
    /// candidate examined straightened successfully.
    /// [`GeodesicCertificate::is_global`] requires all three.
